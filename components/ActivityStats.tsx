...
export function ActivityStats({ activity, history, onSelectRun, isFlawless, isMasterCompleted, isDayOneCompleted, isEpicCompleted, weekOneCompletion, onMarkInvalid }: ActivityStatsProps) {
    ...
    const [localInvalidIds, setLocalInvalidIds] = useState<Set<string>>(() => getInvalidInstanceIds());
    ...
    const activityRuns = useMemo(() => history.filter(h => 
        ...
        !localInvalidIds.has(h.activityDetails.instanceId)
    ...
    useEffect(() => {
        ...
        const checkRuns = async () => {
            for (const run of failedRuns) {
                ...
                if (checkedInstances.has(instanceId) || localInvalidIds.has(instanceId)) {
                    continue;
                }
                ...
                try {
                    ...
                    if (pgcr) {
                        ...
                        if (isSoloFailed || hasTooManyPlayers) {
                            ...
                            setLocalInvalidIds(prev => new Set([...prev, instanceId]));
