...
import { refreshAuthToken } from '@/desktop-app/lib/api-shim';
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        // For desktop app, we retrieve refresh token from accessible cookies
        const refreshToken = Cookies.get('bungie_refresh_token');
        ...
        if (data.access_token) {
           ...
           if (data.refresh_token) {
               Cookies.set('bungie_refresh_token', data.refresh_token, { expires: 90 });
           }
